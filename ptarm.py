...
class Ptarm(LnNode):
    ...
    def get_status(self, peer):
        ...
        try:
            ...
            if ('result' not in jrpc) or ('peers' not in jrpc['result']) or (len(jrpc['result']['peers']) == 0):
                print('  status: none')
                print('get_status1: end: ' + datetime.now().strftime("%Y/%m/%d %H:%M:%S"))
        ...
        print('get_status2: end: ' + datetime.now().strftime("%Y/%m/%d %H:%M:%S") + ', peer=' + peer)
    ...
    def open_channel(self, node_id, amount):
        ...
        cmd = '{"method":"fund","params":["' + node_id + '","0.0.0.0",0,"",0,' + str(amount) + ',0,0,0 ]}'
        print('cmd=' + cmd)
