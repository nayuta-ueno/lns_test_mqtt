...
#
echo payer=ptarmd, payee=cln, no-fulfill
echo ptarmdがunilateral closeする with no-fulfill
#
...
#no-fulfill node(NODE_TYPEには最後に追加する)
port=$((port+2))
NODE_PORT+=(${port})
NO_FULFILL=${port}
NODE_TYPE+=(ptarm)
...
for i in ${NODE_TYPE[@]}; do
	...
	if [ $i == clightning ]; then
		cd ./lightning
		rm -rf rt${port}
		nohup ./lightningd/lightningd --dev-no-reconnect --network=regtest --lightning-dir=rt${port} --addr=${ADDR}:${port} --log-level=debug --rpc-file=/tmp/light${port} > ${LOGDIR}/cln${port}.log&
		PID+=($!)
...
# no-fulfill
echo NO FULFILL PTARMD
../ptarmigan/install/ptarmcli --debug 1 $((NO_FULFILL+1))
...
echo python3 ${TESTSCRIPT} ${TESTNAME} ${NODEID[@]}
python3 ${TESTSCRIPT} ${TESTNAME} ${NODEID[@]}
#nohup python3 ${TESTSCRIPT} ${TESTNAME} ${NODEID[@]} > ${LOGDIR}/mqtt_req.log&
echo "kill -9 $!" >> ${KILLSH}
