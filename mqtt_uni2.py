# encoding: utf-8
'''
node0---node1---ptarmd(no fulfill return)

    while True:
        [node0 => node1]connect
        [node1 => ptarmd]connect
        [node0 => node1]open_channel
        [node1 => ptarmd]open_channel
        ...
        for PAY_COUNT_MAX:
            [ptarmd]invoice
            [node0]pay-node1
...
# ログ用のラベル
NODE_LABEL = [
    'node0', 'node1', 'no-fulfill'
]

# [0]が[1]に向けてconnectする
NODE_CONNECT = [
    [NODE0, NODE1], [NODE1, NOFULFILL]
]
# [0]が[1]に向けてopen_channelする
NODE_OPEN = [
    [NODE0, NODE1], [NODE1, NOFULFILL]
]
# [0]が[1]に向けてcloseする(or [0]がunilateral closeする)
NODE_CLOSE = [
    [NODE0, NODE1], [NODE1, NOFULFILL]
]
NODE_OPEN_AMOUNT = 0
...
def notifier(client):
    while True:
        ...
        if (is_funding == FUNDING_NONE) and (conn_count < CONN_MAX):
...
def proc_status(client, msg, recv_id):
    global dict_status_node, thread_request, loop_reqester,\
           is_funding, funded_block_count, conn_count
...
if __name__ == '__main__':
    if len(sys.argv) != 2 + NODE_NUM:
        print('usage: ' + sys.argv[0] + ' INI_SECTION NODE0 NODE1 NOFULFILL')
        sys.exit()
    for i in range(NODE_NUM):
