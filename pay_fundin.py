...
def create_conf(name, sendtx, newaddr, funding_sat, push_msat):
    ...
    if len(name) > 0:
        ...
        # txid,txindex,funding_satoshis,push_msat,feerate_per_kw
        print('"' + txid + '",' + str(txindex) + ',' + str(funding_sat) + ',' + str(push_msat) + ',' + str(feerate_per_kw))
...
if __name__ == '__main__':
    ...
    if not args[1].isdecimal() or not args[2].isdecimal():
        ...
        sys.exit()
    if int(args[1]) < 10000:
        print('ERROR: funding_satoshis < 10,000 sat')
        sys.exit()
